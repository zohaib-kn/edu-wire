...
import { DestinationPopularCoursesSection } from "@/lib/data/destinations/types";

interface DestinationPopularCoursesProps {
  data?: DestinationPopularCoursesSection;
}

export default function DestinationPopularCourses({
  data,
}: DestinationPopularCoursesProps) {
  if (!data) return null;
      ...
      <div className="max-w-7xl mx-auto px-6 md:px-8">
        ...
        <div className="text-center max-w-3xl mx-auto mb-12 space-y-3">
          ...
          <h2 className="text-3xl md:text-4xl font-extrabold text-slate-950 tracking-tight">
            {data.title.includes("Courses in") ? (
              <>
                {data.title.split("Courses in")[0]}
                <span className="text-[#0D6493]">Courses in Spain</span>
              </>
            ) : (
              data.title
            )}
